		...
		public function __construct(){
			...
			switch($act){
				...
				default:
					...
					$total = $this->model->num_rows("SELECT dt.* FROM tbl_user u join tbl_detai dt on u.pk_user_id = dt.fk_user_id join tbl_bomon bm on bm.pk_mabomon_id = u.fk_mabomon_id WHERE bm.fk_user_id = '$pk_user_id' and dt.c_trangthai in (0) ");
					//tinh so trang
					...
					$arr = $this->model->get_all("SELECT dt.* FROM tbl_user u join tbl_detai dt on u.pk_user_id = dt.fk_user_id join tbl_bomon bm on bm.pk_mabomon_id = u.fk_mabomon_id WHERE bm.fk_user_id = '$pk_user_id' and dt.c_trangthai in (0) order by pk_madetai_id desc limit $from,$record_per_page");
					//load view
					include "view/backend/w_truongbomon/view_detaichoxetduyet.php";
				break;
			}
